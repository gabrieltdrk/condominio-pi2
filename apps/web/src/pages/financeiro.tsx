        ...
        <section className="overflow-hidden rounded-[36px] border border-emerald-100/80 bg-[radial-gradient(circle_at_top_left,_rgba(74,222,128,0.34),_transparent_28%),radial-gradient(circle_at_85%_20%,_rgba(59,130,246,0.16),_transparent_24%),linear-gradient(135deg,_#ecfdf5_0%,_#ffffff_42%,_#eff6ff_100%)] p-6 shadow-[0_38px_90px_-42px_rgba(16,185,129,0.42)]">
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
            <div className="min-w-0">
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700"><Landmark size={13} />Central financeira do condominio</div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">Receitas, despesas, inadimplencia e movimentos recentes organizados em uma visao executiva mais marcante, elegante e facil de agir.</p>
            ...
            <div className="min-w-0 rounded-[32px] border border-slate-900/5 bg-slate-950 p-6 text-white shadow-[0_32px_80px_-38px_rgba(15,23,42,0.8)]">
              ...
              <p className="mt-3 max-w-md text-sm leading-6 text-slate-300">O saldo mostra o que de fato entrou menos o que ja saiu. E a melhor leitura para sentir o folego do condominio.</p>
          ...
          <>
            <section className="grid gap-4 lg:grid-cols-4">
              {[
                ...
                { icon: AlertTriangle, label: "Inadimplencia", value: formatCurrency(delinquencyRows.reduce((sum, item) => sum + item.amount, 0)), helper: "Taxas que ainda pedem cobranca", tone: "bg-[linear-gradient(135deg,_#fff1f2,_#ffffff)] border-rose-100 text-rose-700" },
            ...
            <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.95fr)]">
              ...
              <div className="grid gap-4">
                ...
                <div className={`${panelClass} bg-[linear-gradient(180deg,_#ffffff,_#f8fbff)]`}><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-700"><Building2 size={20} /></div><div><h3 className="m-0 text-base font-semibold text-slate-900">Despesas por categoria</h3><p className="mt-1 text-sm text-slate-500">Os custos que mais impactam a operacao do condominio.</p></div></div><div className="mt-4 h-[220px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={expensesByCategory} layout="vertical" margin={{ top: 0, right: 12, left: 8, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis type="category" dataKey="name" width={105} tick={{ fontSize: 11, fill: "#64748b" }} axisLine={false} tickLine={false} /><Tooltip formatter={(value) => formatCurrency(Number(value ?? 0))} /><Bar dataKey="value" radius={[0, 12, 12, 0]}>{expensesByCategory.map((entry, index) => <Cell key={entry.name} fill={["#0f766e", "#2563eb", "#f59e0b", "#ef4444", "#7c3aed", "#14b8a6", "#64748b"][index % 7]} />)}</Bar></BarChart></ResponsiveContainer></div></div>
              </div>
            </section>
